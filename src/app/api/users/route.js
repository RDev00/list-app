...
export async function PUT(request){
  try {
    ...
    if(!password) return NextResponse.json({ message: "No se ingresaron los datos requeridos", error: "Bad request" }, { status: 400 });
    if(!token) return NextResponse.json({ message: "Credenciales no ingresadas", error: "Unauthorized" }, { status: 401 });
    if(password === newPassword) return NextResponse.json({ message: "No puedes poner la misma contraseña que ya tenías antes", error: "Password same as last used" }, { status: 409 });
    ...
    if(!match) return NextResponse.json({ message: "Las contraseñas no coinciden", error: "Wrong password" }, { status: 401 });
...
export async function DELETE(request){
  try {
    ...
    if(deleteUserError) return NextResponse.json({ message: "Ha ocurrido un error al querer borrar la cuenta del usuario", error: deleteUserError.message }, { status: 500 });
